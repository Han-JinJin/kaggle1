...
# --- Must be set BEFORE importing transformers/tensorflow to avoid TF DLL issues on Windows ---
os.environ.setdefault("TRANSFORMERS_NO_TF", "1")
os.environ.setdefault("TRANSFORMERS_NO_FLAX", "1")
os.environ.setdefault("USE_TF", "0")
os.environ.setdefault("USE_FLAX", "0")
# W&B can use a background service; thread mode is often more stable with multiprocessing on Windows
...
class CFG:
    ...
    def from_args(args):
        ...
        # --- Model-specific interface fixes ---
        # VideoMAE in this repo predicts logits at (tile_size//16) resolution.
        # Therefore labels are downsampled by scale_factor=16 in VideoDataset.
        if cfg.model == 'vmae':
            if getattr(cfg, 'scale_factor', None) != 16:
                print(f"[CFG] model=vmae: overriding scale_factor {getattr(cfg, 'scale_factor', None)} -> 16")
            cfg.scale_factor = 16
            # We treat the depth stack as frames; each frame is grayscale.
            cfg.out_chans = 1
...
def get_model(cfg, pred_shape, wandb_logger):
    ...
    model_params = {
        ...
        'size': cfg.size,
        'lr': cfg.lr,
        'in_chans': cfg.in_chans,  # Add in_chans parameter
    ...
    if cfg.model == 'swin':
        ...
        # VideoDataset always outputs T=cfg.valid_chans frames (train uses channel-crop augmentation).
        # So the model must be configured with num_frames == cfg.valid_chans.
        model_params['in_chans'] = cfg.valid_chans
...
def train_single_fold(cfg, segments, valid_id):
    ...
    # Avoid reusing an old W&B run (common if a previous run crashed)
    if wandb.run is not None:
        wandb.finish()
